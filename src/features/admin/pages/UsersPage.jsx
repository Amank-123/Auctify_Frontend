...
function Avatar({ src, name }) {
    ...
    return (
        <div
            className={`w-9 h-9 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-white text-xs font-bold ring-2 ring-white shadow-sm flex-shrink-0`}
        >
            {initials}
        </div>
    );
...
function RoleBadge({ role }) {
    ...
    return (
        <span
            className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[11px] font-semibold ring-1 capitalize ${map[role] || map.user}`}
        >
            {role}
        </span>
    );
...
const UsersPage = () => {
    ...
    useEffect(() => {
        ...
        setFiltered(
            users.filter(
                (u) => u.username?.toLowerCase().includes(v) || u.email?.toLowerCase().includes(v),
            ),
        );
    }, [search, users]);
            ...
            <div className="relative z-10 max-w-5xl mx-auto">
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {/* Search */}
                        <div
                            className={`flex items-center gap-2 rounded-xl px-3 py-2 border text-sm transition-all duration-200 ${
                                searchFocused
                ...
                >
                    <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3.5 py-2 shadow-sm">
                        ...
                        <span className="mono text-sm font-bold text-slate-800">
                            {users.length}
                        </span>
                    </div>
                    <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-3.5 py-2">
                        ...
                        <span className="mono text-sm font-bold text-emerald-700">
                            {totalActive}
                        </span>
                    </div>
                    ...
                    {search && (
                        <div className="flex items-center gap-2 bg-violet-50 border border-violet-200 rounded-xl px-3.5 py-2">
                            ...
                            <span className="mono text-sm font-bold text-violet-700">
                                {filtered.length}
                            </span>
                        </div>
                    )}
                ...
                >
                    ...
                    <div className="grid grid-cols-[2fr_2fr_1fr_1fr_1fr] px-5 py-3 border-b border-slate-100 bg-slate-50">
                        {["User", "Email", "Role", "Status", "Action"].map((h, i) => (
                            <p
                                key={h}
                                className={`text-[10px] font-bold uppercase tracking-widest text-slate-400 ${i === 4 ? "text-right" : ""}`}
                            >
                                {h}
                            </p>
                        ))}
                                ...
                                {filtered.map((user, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div>
                                                <span
                                                    className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-[11px] font-semibold ring-1 ${
                                                        user.isBanned
