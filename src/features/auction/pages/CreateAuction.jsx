      ...
      <div className="max-w-2xl mx-auto relative">
        ...
        <div className="mb-12">
          ...
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-tight">Create New Auction</h1>
              <p className="text-gray-500 mt-1">List your item and let collectors bid</p>
            </div>
          </div>
            ...
            {["Item Details", "Pricing & Schedule", "Media Upload"].map((label, i) => (
              <div key={i} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center">
                  <div className={`w-9 h-9 rounded-2xl flex items-center justify-center text-sm font-semibold border-2 transition-all
                    ${i + 1 < step
                  ...
                  <span className={`text-xs font-medium mt-2 text-center transition-colors ${i + 1 <= step ? "text-gray-700" : "text-gray-400"}`}>
                    {label}
                  </span>
                </div>
                {i < 2 && (
                  <div className={`flex-1 h-px mx-4 mt-4 transition-all ${i + 1 < step ? "bg-orange-500" : "bg-gray-200"}`} />
                )}
              </div>
            ))}
          </div>
                ...
                >
                  <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0}>
                    ...
                    <div className="relative">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-2xl text-gray-400 font-light">₹</span>
                      <input
                        type="number"
                        name="startPrice"
                  ...
                  >
                    <div className="mx-auto w-16 h-16 bg-orange-100 rounded-2xl flex items-center justify-center mb-4">
                      <svg className="w-9 h-9 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M4 16v-4m0 0l4 4m-4-4l4-4m12 0v4m0 0l-4-4m4 4l-4 4" />
                      </svg>
                    </div>
                    ...
                    <p className="text-xs text-gray-400 mt-3">PNG, JPG, JPEG • Max 10MB per image</p>
                    <input type="file" multiple accept="image/*" onChange={handleFileChange} className="hidden" />
                  </motion.label>

                  {preview.length > 0 && (
                    <div>
                      <p className="text-sm font-medium text-gray-600 mb-3">Uploaded Images ({preview.length})</p>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {preview.map((src, i) => (
                          ...
                          >
                            <img src={src} alt="preview" className="w-full aspect-square object-cover" />
                            {i === 0 && (
