...
const STATUS = {
  active:  { label: "● Live",    cls: "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" },
  draft:   { label: "◌ Draft",   cls: "bg-zinc-700/60     text-zinc-400   border border-zinc-600/30" },
  ended:   { label: "✕ Ended",   cls: "bg-orange-500/10  text-orange-400 border border-orange-500/20" },
  expired: { label: "⚠ Expired", cls: "bg-amber-500/10   text-amber-400  border border-amber-500/20" },
};
...
function StatCard({ label, value, accent, delay }) {
  ...
  return (
    ...
    >
      ...
      />
      <p className="text-xs font-semibold tracking-widest uppercase text-zinc-500 mb-4">{label}</p>
    ...
    >
      ...
      <motion.div
        className={`absolute left-0 top-0 bottom-0 w-1 ${
          ...
          auction.status === "draft"  ? "bg-zinc-600"   :
          "bg-orange-500"
        }`}
        initial={{ scaleY: 0 }}
        animate={{ scaleY: 1 }}
      ...
      <div className="flex gap-5 p-5 pl-6">
        ...
        >
          <img
            src={auction.media?.[0] || "https://via.placeholder.com/200/1a1a1a/444?text=IMG"}
            alt={auction.name}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-3 mb-1">
            ...
            <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full tracking-wide ${cfg.cls}`}>
              {cfg.label}
            </span>
          </div>
          ...
          <div className="flex gap-8 text-sm">
            {[
              { lbl: "Starting", val: fmtINR(auction.startPrice), color: "text-zinc-300" },
              { lbl: "Highest Bid", val: fmtINR(auction.currentHighestBid), color: "text-emerald-400 font-bold" },
              { lbl: "Bids", val: auction.bidCount ?? 0, color: "text-orange-400 font-bold" },
            ].map(({ lbl, val, color }) => (
              <div key={lbl}>
                <span className="block text-[10px] uppercase tracking-widest text-zinc-600 mb-0.5">{lbl}</span>
...
function Skeleton() {
  return (
    <div className="space-y-4">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex gap-5 animate-pulse">
...
export default function SellerDashboard() {
  ...
  const handleStart = async (id) => {
    try {
      ...
      fetchAuctions();
    } catch (err) { showError(err.response?.data?.message || "Failed to start auction"); }
  };
  ...
  const handleEnd = async (id) => {
    try {
      ...
      fetchAuctions();
    } catch (err) { showError(err.response?.data?.message || "Failed to end auction"); }
  };
        ...
        >
          ...
          <div className="px-6 pt-8 pb-6 border-b border-zinc-800/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-black text-white text-lg leading-none display">AuctionHub</div>
                <div className="text-[10px] text-zinc-500 tracking-widest uppercase mt-0.5">Seller Portal</div>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-6xl mx-auto px-8 py-10">
                ...
                >
                  <div className="flex items-center gap-3 bg-emerald-500/5 border border-emerald-500/20 rounded-xl px-5 py-3.5">
                    ...
                    <p className="text-emerald-400 text-sm font-medium">
                      <span className="font-bold">{stats.live} auction{stats.live > 1 ? "s" : ""}</span> actively receiving bids right now
                    </p>
                  </div>
                </motion.div>
              )}
            ...
            {loading ? (
              ...
              >
                ...
                <p className="text-zinc-600 text-sm mb-6">Try adjusting your search or filter</p>
                <Link to="/auction/create" className="text-orange-400 text-sm font-semibold hover:text-orange-300 transition-colors">
